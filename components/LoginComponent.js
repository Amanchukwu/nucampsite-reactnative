...
class LoginTab extends Component { //Renamed from "Login" to "LoginTab" in order to implement the bottom tab navigator from react-navigation-tabs
    ...
    static navigationOptions = {
        title: 'Login',
        tabBarIcon: ({tintColor}) => (  //added for bottom tab navigator. Set to a function to which we pass the tint color. THis funciton automatically receives "tintColor" as an argument from the bottomtabnavigator component which we will create and define the tint colors for. Will return an <Icon> component as seen below.
            <Icon
                name='sign-in'
                type='font-awesome'
    ...
    render() {
        return (
            <View style={styles.container}>
                ...
                <View style={styles.formButton}>
                    <Button
                        onPress={() => this.props.navigation.navigate('Register')} //Want button to route to the register screen which can be done using the navigation props ".navigate" function. When using the ".navigate" funcion from the Navigation Props, could destructure as done before, or do in the way written in this line.
                        title='Register'
...
class RegisterTab extends Component { //Creating the RegisterTab component.

    constructor(props) {
        super(props);

        this.state = { //Constructor and state needed because the form for RegisterTab will be a controlled form, where the data is stored within the component's state
            username: '',
            password: '',
            firstname: '',
            lastname: '',
    ...
    static navigationOptions = { //Copied from the "LoginTab" component
        title: 'Register',
    ...
    getImageFromCamera = async () => { //Set up as an arrow function so that the corresponding <Button>'s on press can be written cleanly like this: "onPress={this.getImageFromCamera}". Set up as an "async" method so we can use "await" inside of it so we can handle promises.
        ...
        const cameraRollPermission = await Permissions.askAsync(Permissions.CAMERA_ROLL);//Put the second permission inside a vairable called "cameraRollPermission". Inside it, await the resolve value of a promise from "Permissions.askAsync" for "Permissions.CAMERA_ROLL". This will allow us to read from and write to the cameraRoll.
        ...
        if (cameraPermission.status === 'granted' && cameraRollPermission.status === 'granted') { //when the promise returns from the "Permissions.askAsync" method, it resolves into an object that contains a "status" property which will be granted if the request was successful. 'if' condition will check if the "cameraPermission.status" and "cameraRollPermission.status" variables we created contains the status that is equal to the string 'granted'.
            const capturedImage = await ImagePicker.launchCameraAsync({ //if permissions were granted, set up a variable that will ultimately hold the local uri (the local filepath of the captured image).  THis is the value that will be resolved from the promise returned from "ImagePicker.launchCameraAsync". This method takes a single argument, an object with option configurations.
    ...
    handleRegister() { //Function to handle the form submission. Since there is no back end, this method/function will console log the input information. If the remember checkbox is picked, it will add the input information to the SecureStore. Copied from the "handleLogin" method/funciton and name just changed to "handleRegister" 
        console.log(JSON.stringify(this.state));
    ...
    render() {
        return ( //Create the register form as a controlled form where the information is saved in the component's state. Copied from "loginTab"'s form.
            <ScrollView>
                <View style={styles.container}>
...
const Login = createBottomTabNavigator( //Constant called "Login" set to 'react-navigation-tabs' "createBottomTabNavigator" function. Pass it the arguments 2 objects: a list of tabs for the navigator, 2nd for extra options.
    { //1st argument of the list of tabs for the navigator
        Login: LoginTab,
        Register: RegisterTab
    },
    {//Optional 2nd argument 
        tabBarOptions: { //Colors for the tab bar at the bottom of the screen and other formatting options
